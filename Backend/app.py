...
def loans_info():
    ...
    if method == 'GET':
        loans = [{'id': loan.id, 'cust_id': loan.cust_id, 'book_id': loan.book_id, 'loan_date': str(loan.loan_date), 'return_date': str(loan.return_date), 'cust_name': loan.customer.name, 'book_title': loan.book.title} for loan in loan_dal.get_all_loans()]
...
def loan_info(id):
    ...
    if method == 'GET':
        ...
        return jsonify({'id': loan.id, 'cust_id': loan.cust_id, 'book_id': loan.book_id, 'cust_name': loan.customer.name, 'book_title': loan.book.title, 'loan_date': str(loan.loan_date), 'return_date': str(loan.return_date)}), 200
    elif method == 'POST':
        if identity == None:
            return jsonify({'message': 'An account is required to use this method'}), 401
...
def late_loans():
    ...
    if method == 'GET':
        loans = [{'id': loan.id, 'cust_id': loan.cust_id, 'book_id': loan.book_id, 'loan_date': str(loan.loan_date), 'return_date': str(loan.return_date), 'cust_name': loan.customer.name, 'book_title': loan.book.title} if loan.is_late() else None for loan in loan_dal.get_all_loans()]
        return jsonify(loans), 200
